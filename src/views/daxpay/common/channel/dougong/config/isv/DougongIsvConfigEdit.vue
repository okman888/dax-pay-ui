  ...
  >
    <a-spin :spinning="confirmLoading">
      ...
      >
        ...
        <!--        </a-form-item>-->
        <a-form-item label="服务商系统ID" name="sysId">
          <a-input v-model:value="form.sysId" placeholder="请输入服务商系统ID" />
        </a-form-item>
        <a-form-item label="产品号" name="productId">
          <a-input v-model:value="form.productId" placeholder="请输入产品号" />
        </a-form-item>
        <a-form-item label="斗拱公钥" name="dgPublicKey">
          <a-textarea v-model:value="form.dgPublicKey" placeholder="请输入斗拱公钥" :rows="5" />
        </a-form-item>
...
<script lang="ts" setup>
  ...
  import { update, getConfig, DougongIsvConfig } from './DougongIsvConfig.api'
  ...
  const form = ref<DougongIsvConfig>({
    enable: true,
    sandbox: false,
  })
  let rawForm: any = {}
  ...
  const rules = computed(() => {
    return {
      ...
      sysId: [{ required: true, message: '请输入服务商系统ID' }],
      productId: [{ required: true, message: '请输入产品号' }],
      privateKey: [{ required: true, message: '请输入商户私钥' }],
      dgPublicKey: [{ required: true, message: '请输入斗拱公钥' }],
