...
<script setup lang="ts">
  ...
  import { CheckoutCounterConfig, listByType, deleteConfig } from './CheckoutCounterConfig.api'
  ...
  const records = ref<CheckoutCounterConfig[]>([])
  ...
  function queryPage() {
    loading.value = true
    listByType(props.appId, props.type)
      .then(({ data }) => {
        records.value = data
        loading.value = false
      })
   ...
   * 编辑
   */
  function edit(record: CheckoutCounterConfig) {
   ...
   * 查看
   */
  function show(record: CheckoutCounterConfig) {
   ...
   * 删除
   */
  function del(record: CheckoutCounterConfig) {
    createConfirm({
      iconType: 'warning',
      title: '删除',
      content: '是否删除该数据',
