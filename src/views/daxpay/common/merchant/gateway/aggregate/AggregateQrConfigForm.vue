...
<script setup lang="ts">
  ...
  import { AggregateQrPayConfig, findQrByAppId, updateQrConfig } from './AggregateConfig.api'
  ...
  const form = ref<AggregateQrPayConfig>({
    autoLaunch: true,
  })

  // 表单验证规则
  const rules = {
    autoLaunch: [{ required: true, message: '是否自动启动必选' }],
  ...
  function initData() {
    ...
    edit.value = false
    // 根据应用ID查询配置
    findQrByAppId(props.appId).then(({ data }) => {
      form.value = data
